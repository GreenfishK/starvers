...
for v in $(seq 0 1 1298); do
    ...
    time docker run -it --rm \
        ...
        rfdhdt/hdt-cpp rdf2hdt -f ntriples $fileadd /var/data/out/cb/$v.add.hdt > output/load-bearb-hour-cb-$v-add-hdt.txt
    time docker run -it --rm \
        -v $(pwd)/bearb-hour-hdt/:/var/data/out/ \
        -v $(pwd)/rawdata-bearb/hour/:/var/data/in/ \
        rfdhdt/hdt-cpp rdf2hdt -f ntriples $filedel /var/data/out/cb/$v.del.hdt > output/load-bearb-hour-cb-$v-del-hdt.txt
done
