...
if [[ "$reset" == "true" ]]; then
    ...
    rm -rf /starvers_eval/databases/preprocessing/jenatdb2/${policy}_${dataset}
    rm -rf /starvers_eval/configs/preprocessing/jenatdb2/${policy}_${dataset}
    rm -rf /run/configuration

    echo "$(log_timestamp) ${log_level}:Create directories..." >> $log_file
    mkdir -p /starvers_eval/configs/preprocessing/jenatdb2/${policy}_${dataset}
    mkdir -p /run/configuration

    echo "$(log_timestamp) ${log_level}:Parametrize and copy config file..." >> $log_file
    repositoryID=${policy}_${dataset}
    cp ${script_dir}/2_preprocess/configs/jenatdb2-config_template.ttl /starvers_eval/configs/preprocessing/jenatdb2/${policy}_${dataset}/${repositoryID}.ttl
    sed -i "s/{{repositoryID}}/$repositoryID/g" /starvers_eval/configs/preprocessing/jenatdb2/${policy}_${dataset}/${repositoryID}.ttl
fi

if [[ "$ingest_empty" == "true" ]]; then
    echo "$(log_timestamp) ${log_level}:Ingest empty dataset..." >> $log_file
    /jena-fuseki/tdbloader2 --loc /starvers_eval/databases/preprocessing/jenatdb2/${policy}_${dataset}/${repositoryID} /starvers_eval/rawdata/${dataset}/empty.nt
fi
...
cp /starvers_eval/configs/preprocessing/jenatdb2/${policy}_${dataset}/*.ttl /run/configuration
