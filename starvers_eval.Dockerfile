...
# Build ostrich-node and link it
...
COPY --from=install_ostrich /usr/local/bin/node /usr/local/bin/node
COPY --from=install_ostrich /usr/local/lib/node_modules /usr/local/lib/node_modules
COPY --from=install_ostrich /usr/local/share/.config/yarn /usr/local/share/.config/yarn
...
# Set pythonpath for module imports relative to PYTHONPATH
