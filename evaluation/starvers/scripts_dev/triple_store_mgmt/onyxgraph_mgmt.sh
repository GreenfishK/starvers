...
# Logging variables
log_file=${RUN_DIR}/output/logs/onyx_graph.txt
...
shutdown() {
    ...
    # Final check
    if ss -ltnp | grep -q ':7878'; then
        echo "$(log_timestamp) ${log_level}:ERROR - Port 7878 still in use after timeout" >> "$log_file"
        exit 1
    fi
...
ingest_empty() {
    ...
    /root/.cargo/bin/oxigraph load --location ${database_dir} --file ${RUN_DIR}/rawdata/${dataset}/empty.nt  >> $log_file 2>&1
}
